...
export function doBotAction(state) {
    ...
    if (state.status === 'selecting_pain') {
        const botToSelect = state.players.find((p) => p.isBot && p.chosenPainCard === null);
        if (botToSelect) {
...
function choosePainCardForBot(bot) {
    const config = bot.botConfig || BOT_ARCHETYPES.Average;
    const zeros = bot.hand.filter((c) => c.value === 0);
    if (zeros.length > 0) {
        if (Math.random() < config.skill + 0.2)
            return zeros[0];
    }
    const colorCounts = {};
    bot.hand.forEach((c) => {
...
function evaluatePainCard(card, countInHand) {
    if (card.value === 0)
        return -100;
    return card.value + (countInHand * 10);
}
function playTrickCardForBot(bot, state) {
    const config = bot.botConfig || BOT_ARCHETYPES.Average;
    const hand = [...bot.hand];
